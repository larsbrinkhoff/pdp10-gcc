     ...
     int *new_direct_jump_p;
{
  ...
  if (i1 == 0
	...
	{
	  /* Handle the case of the higher word not fitting entirely
	     in either hi or lo.  */

	  lo &= (UWIDE_SHIFT_LEFT_BY_BITS_PER_WORD (1) - 1);
	  lo |= (UWIDE_SHIFT_LEFT_BY_BITS_PER_WORD
		 (INTVAL (SET_SRC (PATTERN (i3)))));
	  hi = (INTVAL (SET_SRC (PATTERN (i3))) << (HOST_BITS_PER_WIDE_INT - BITS_PER_WORD))
	    >> (2 * (HOST_BITS_PER_WIDE_INT - BITS_PER_WORD));
	}
     ...
     int in_dest, in_compare;
{
  ...
  enum machine_mode wanted_inner_mode = word_mode;
  ...
  if (tmode != BLKmode
      ...
      /* PDP-10 can't address non-word-aligned data.  */
      && ((pos == 0 && GET_MODE_SIZE (inner_mode) < UNITS_PER_WORD)
	  || REAL_BITS_PER_UNIT != BITS_PER_WORD))
    {
      ...
      if (GET_CODE (inner) == MEM)
	{
	  ...
	  offset /= REAL_BITS_PER_UNIT / BITS_PER_UNIT;
	...
	case MEM:
	  ...
	  if ((code == ASHIFTRT || code == LSHIFTRT)
	      ...
	      && (tmode = mode_for_size (GET_MODE_BITSIZE (mode) - count,
					 MODE_INT, 1)) != BLKmode)
	    {
#if REAL_BITS_PER_UNIT == BITS_PER_UNIT
