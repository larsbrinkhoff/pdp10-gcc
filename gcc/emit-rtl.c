     ...
     rtx x;
{
  ...
  /* If X is a CONST_INT or a CONST_DOUBLE, extract the appropriate bits
	...
	{
	  /* gcc.c-torture/execute/920501-9.c: CONST_DOUBLE -> DImode */
	  /* gcc.c-torture/execute/930208-1.c: CONST_DOUBLE -> DImode */
	  /* gcc.c-torture/execute/950503-1.c: CONST_DOUBLE -> DImode */
	  /* gcc.c-torture/execute/950511-1.c: CONST_DOUBLE -> DImode */
	  /* gcc.c-torture/execute/950704-1.c: CONST_DOUBLE -> DImode */
	  /* gcc.c-torture/unsorted/fq.c: CONST_INT -> DImode */

	  /* A CONST_INT can never overflow a mode that is wider than
	     HOST_BITS_PER_WIDE_INT.  */
	  if (GET_CODE (x) == CONST_INT)
	    return x;
	  else
	    {
	      /* A CONST_DOUBLE with an appropriate number of sign
                 extension bits is ok.  */
	      HOST_WIDE_INT sign_bits = CONST_DOUBLE_HIGH (x);
	      sign_bits >>= GET_MODE_BITSIZE (mode) - HOST_BITS_PER_WIDE_INT;
  ...
  /* Single-precision floats are always 32-bits and double-precision
     floats are always 64-bits.  */
	   ...
	   && (GET_MODE_BITSIZE (mode) == 32 || GET_MODE_BITSIZE (mode) == 36)
	   && GET_CODE (x) == CONST_INT)
  {
      ...
      return CONST_DOUBLE_FROM_REAL_VALUE (r, mode);
	   ...
	   && (GET_MODE_BITSIZE (mode) == 64 || GET_MODE_BITSIZE (mode) == 72)
	   && (GET_CODE (x) == CONST_INT || GET_CODE (x) == CONST_DOUBLE)
	   && GET_MODE (x) == VOIDmode)
    {
      REAL_VALUE_TYPE r;
     ...
     rtx x;
{
  ...
  if (result)
	...
	/* PDP-10: pretend it's a bit field.  */
	return
	  gen_rtx_SUBREG (mode,
			  gen_rtx_ZERO_EXTRACT (word_mode, x,
						GEN_INT (GET_MODE_BITSIZE (mode)),
						GEN_INT (offset * BITS_PER_UNIT)),
			  0);
      else
	return adjust_address (x, mode,
...
#if HOST_BITS_PER_WIDE_INT >= 64
      else if (BITS_PER_WORD == 36 && offset == 0)
	{
	  val = k[0];
	  val = (((val & 0xffffffff) ^ 0x80000000) - 0x80000000) << 4;
	  val |= (HOST_WIDE_INT) (k[1] >> 28) & 0xf;
	  return GEN_INT (val);
	}
...
#else
  if (offset / size_ratio == 0)
    val = GET_CODE (op) == CONST_INT ? INTVAL (op) : CONST_DOUBLE_LOW (op);
  else if (GET_CODE (op) == CONST_INT)
    val = INTVAL (op) >> BITS_PER_WORD;
  else
    val = (((unsigned HOST_WIDE_INT) CONST_DOUBLE_LOW (op)) >> BITS_PER_WORD)
      + (CONST_DOUBLE_HIGH (op) << (HOST_BITS_PER_WIDE_INT - BITS_PER_WORD));
#endif
     ...
     enum machine_mode mode;
{
  ...
  if (GET_CODE (op) == MEM)
    {
      rtx new = adjust_address_nv (op, word_mode,
...
static rtx
change_address_1 (memref1, mode, addr, validate)
     rtx memref1;
     enum machine_mode mode;
     rtx addr;
     int validate;
...
rtx
change_address (memref1, mode, addr)
     rtx memref1;
     enum machine_mode mode;
     rtx addr;
{
  rtx new = change_address_1 (memref1, mode, addr, 1);
...
rtx
adjust_address_1 (_memref, mode, offset, validate, adjust)
     rtx _memref;
     enum machine_mode mode;
...
{
  rtx memref =
    GET_CODE (_memref) == SUBREG
    && GET_CODE (XEXP (_memref, 0)) == ZERO_EXTRACT
    ? XEXP (XEXP (_memref, 0), 0) : _memref;
  ...
  if (offset != 0)
    memalign
      = MIN (memalign,
	     /* PDP-10: offset is number of words.  */
	     (unsigned HOST_WIDE_INT) (offset & -offset) * REAL_BITS_PER_UNIT);
  ...
  if (GET_CODE (_memref) == SUBREG
      && GET_CODE (SUBREG_REG (_memref)) == ZERO_EXTRACT)
    {
      PUT_MODE (new, SImode);
      new = gen_rtx_ZERO_EXTRACT (SImode, new,
				  XEXP (SUBREG_REG (_memref), 1),
				  XEXP (SUBREG_REG (_memref), 2));
...
#ifdef __PDP10_H__
  /* PDP-10 offsets are in words, not memory units.  */
  pow2 *= UNITS_PER_WORD;
#endif
