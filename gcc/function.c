     ...
     struct function *function;
{
  ...
  /* Allow the target to (possibly) increase the size of this stack
     slot.  */
  type = (*lang_hooks.types.type_for_mode) (mode, 0);
  if (type)
  ...
  /* PDP-10.  */
  {
    ...
    x = gen_rtx_MEM (mode, plus_constant (base, offset / 4));
	...
	x = gen_rtx_ZERO_EXTRACT (word_mode, x,
				  ...
				  GEN_INT (pos));
	x = gen_rtx_SUBREG (mode, x,
			    GET_MODE_SIZE (word_mode) - GET_MODE_SIZE (mode));
      }
     ...
     tree type;
{
  ...
  if (p == 0)
	...
	abort ();
      /* PDP-10: special hack for arrays.  */
  ...
  RTX_UNCHANGING_P (p->slot) = 0;
  {
    ...
    set_mem_alias_set (mem, type ? get_alias_set (type) : 0);
	...
	MEM_VOLATILE_P (mem) = TYPE_VOLATILE (type);
	MEM_SET_IN_STRUCT_P (mem, AGGREGATE_TYPE_P (type));
      }
  }
     ...
     struct hash_table *ht;
{
  ...
  PUT_CODE (reg, GET_CODE (new));
  PUT_MODE (reg, decl_mode);
  XEXP (reg, 0) = XEXP (new, 0);
  if (GET_CODE (new) == SUBREG)
    XINT (reg, 1) = XINT (new, 1);
  {
    rtx mem = reg;
    if (GET_CODE (reg) == SUBREG && GET_CODE (XEXP (reg, 0)) == ZERO_EXTRACT)
      mem = XEXP (XEXP (reg, 0), 0);
    MEM_ATTRS (mem) = 0;
     ...
     rtx no_share;
{
  ...
  switch (code)
      ...
      if (XEXP (x, 0) == var)
	{
	  ...
	  /* PDP-10.  */
	  if (GET_CODE (var) == SUBREG
	      && GET_CODE (SUBREG_REG (var)) == ZERO_EXTRACT
	      && GET_CODE (XEXP (SUBREG_REG (var), 0)) == MEM)
	    {
	      int bitsize = INTVAL (XEXP (sub, 1));
	      int bitpos = INTVAL (XEXP (sub, 2));
		...
		abort ();
	      ...
	      start_sequence ();
	      emit_move_insn (tmp, XEXP (XEXP (sub, 0), 0));

	      reg = XEXP (XEXP (SUBREG_REG (var), 0), 0);
	      if (GET_CODE (reg) == PLUS)
		reg = XEXP (reg, 0);
	      if (! REGNO_PTR_FRAME_P (REGNO (reg)))
		emit_insn (gen_iorsi3 (tmp, tmp,
	    ...
	    sub = gen_rtx_UNSPEC (Pmode, gen_rtvec (1, sub), UNSPEC_ADDRESS);
  ...
  for (parm = fnargs; parm; parm = TREE_CHAIN (parm))
	  ...
	  )
	{
	 ...
	 0 means it arrives on the stack.  */
      ...
      if (entry_parm == 0)
	promoted_mode = passed_mode;
      else if (POINTER_TYPE_P (passed_type))
	mark_reg_pointer (entry_parm, TYPE_ALIGN (TREE_TYPE (passed_type)));
	...
	if (offset_rtx == const0_rtx)
	  ...
	  {
	    /* PDP-10: offset in words, not units.  */
	    offset_rtx = GEN_INT (INTVAL (offset_rtx)
      ...
      if (entry_parm)
	{
	  ...
	  if (nregs > 0)
	    {
	      rtx mem = validize_mem (stack_parm);

#ifndef STACK_GROWS_DOWNWARD
	      /* If the stack grows upward, copy to the end of the
		 stack space.  */
	      mem = adjust_address (mem, GET_MODE (mem),
				    int_size_in_bytes (TREE_TYPE (parm)) / UNITS_PER_WORD
				    - nregs);
#endif
		...
		emit_group_store (mem, entry_parm,
				  int_size_in_bytes (TREE_TYPE (parm)));

	      else
		move_block_from_reg (REGNO (entry_parm), mem, nregs,
				     int_size_in_bytes (TREE_TYPE (parm)));
	       ...
	       || passed_pointer || parm == function_result_decl)
	{
	  ...
	  /* ??? Later add code to handle the case that if the argument isn't
		  ...
		  || TREE_CODE (TYPE_SIZE (type)) != INTEGER_CST)
		{
		  ...
		  rtx address
		    = allocate_dynamic_stack_space (expr_size (parm), NULL_RTX,
						    TYPE_ALIGN (type));
