     ...
     tree strings;
{
  ...
  if (flag_const_strings && ! flag_writable_strings)
	...
	= build_type_variant (wide_flag ? wchar_type_node : char_type_node,
      ...
      /* PDP-10: string constants may have 7-bit bytes.  */
      if (pdp10_string_literal_byte_size == 7)
	elements = build_type_variant (pdp10_char7_unsigned_type_node, 1, 0);
    ...
    TREE_TYPE (value)
      = build_array_type (wchar_type_node,
			  build_index_type (build_int_2 (nchars - 1, 0)));
  else
    {
      /* PDP-10: string constants may have 7-bit bytes.  */
      ...
      switch (pdp10_string_literal_byte_size)
	{
	case 6:
     ...
     tree ptrop, intop;
{
  ...
  if (pdp10_bytesize (TREE_TYPE (TREE_TYPE (ptrop))) >= BITS_PER_WORD)
    size_exp = fold (build (FLOOR_DIV_EXPR, TREE_TYPE (size_exp), size_exp,
			    build_int_2 (UNITS_PER_WORD, 0)));
  else if (pdp10_bytesize (TREE_TYPE (TREE_TYPE (ptrop))) == BITS_PER_WORD / 2)
    size_exp = fold (build (FLOOR_DIV_EXPR, TREE_TYPE (size_exp), size_exp,
			    build_int_2 (2, 0)));
