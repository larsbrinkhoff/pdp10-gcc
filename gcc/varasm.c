     ...
     int dont_output_data;
{
     ...
     (symbol_ref) (const_int)))).  */
  {
	...
	&& (GET_CODE (sym = XEXP (decl_rtl, 0)) == SYMBOL_REF
	    || (GET_CODE (XEXP (decl_rtl, 0)) == CONST
		&& GET_CODE (XEXP (XEXP (decl_rtl, 0), 0)) == PLUS
		&& GET_CODE (sym = XEXP (XEXP (XEXP (decl_rtl, 0), 0), 0))
		== SYMBOL_REF)))
      name = XSTR (sym, 0);
    else
      abort ();
  ...
  if (DECL_P (decl) && DECL_EXTERNAL (decl) && TREE_PUBLIC (decl))
    {
	 ...
	 (mem (const (plus (symbol_ref) (const_int)))).  */
	  ...
	  && (GET_CODE (sym = XEXP (rtl, 0)) == SYMBOL_REF
	      || (GET_CODE (XEXP (rtl, 0)) == CONST
		  && GET_CODE (XEXP (XEXP (rtl, 0), 0)) == PLUS
		  && GET_CODE (sym = XEXP (XEXP (XEXP (rtl, 0), 0), 0))
		  == SYMBOL_REF))
	  && ! SYMBOL_REF_USED (sym))
	{
     ...
     int defer;
{
  ...
  if (desc == 0)
    {
	 ...
	 future calls to this function to find.  */
      ...
      {
	/* PDP-10: handle byte pointers.  */
	rtx addr = gen_rtx_SYMBOL_REF (Pmode, desc->label);
	int ps;

	if (!TARGET_EXTENDED)
	  abort ();

	switch (pdp10_bytesize (TREE_TYPE (exp)))
	  {
	  case 6:
	    ps = 046;
	    break;
	...
	addr = plus_constant (addr,
			      trunc_int_for_mode ((HOST_WIDE_INT)ps << 30,
						  SImode));
	rtl = desc->rtl
	  = gen_rtx_MEM (TYPE_MODE (TREE_TYPE (exp)), addr);
      }
  ...
  if (! found)
	...
	ENCODE_SECTION_INFO (exp, true);
      ...
      desc->rtl = rtl;
      if (GET_CODE (XEXP (desc->rtl, 0)) == CONST
	  && GET_CODE (XEXP (XEXP (desc->rtl, 0), 0)) == PLUS)
	desc->label = XSTR (XEXP (XEXP (XEXP (desc->rtl, 0), 0), 0), 0);
      else
	  ...
	  if (! flag_syntax_only)
	      ...
	      else
		{
		  ...
		  *defstr = p;
		  if (GET_CODE (XEXP (desc->rtl, 0)) == CONST
		      && GET_CODE (XEXP (XEXP (desc->rtl, 0), 0)) == PLUS)
		    STRING_POOL_ADDRESS_P (XEXP (XEXP (XEXP (desc->rtl, 0), 0), 0)) = 1;
		  else
