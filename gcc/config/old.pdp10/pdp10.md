...
(define_insn "negdi2"
  [(set (match_operand:DI 0 "reg_or_mem_operand" "=r,m")
	(neg:DI (match_operand:DI 1 "reg_or_mem_operand" "rm,r")))]
  ;; 71-bit arithmetic.
  "0 && TARGET_KI10up"
  "@
   dmovn %0,%1
...
(define_insn "zero_extendhisi2"
  ...
  ""
  {
    if (zero_extended_p (operands[0], 18, insn)
	&& REGNO (operands[0]) == REGNO (operands[1]))
      return "";
    else
...
(define_insn "*call_mem1"
  [(call (mem:SI (match_operand:SI 0 "memory_operand" "o"))
...
;; These also work for long long.

;; To compare a long long or double against zero:
;; x < 0:
;;	SKIPL x
;;	 ;here if false
;; x >= 0:
...
(define_insn "test_extract_skip"
  ...
  ""
  {
    ...
    HOST_WIDE_INT mask = (((HOST_WIDE_INT)1 << len) - 1) << (36 - pos - 1);
...
(define_expand "allocate_stack"
  ...
  "{
     ...
     if (GET_CODE (operands[1]) == CONST_INT)
       ...
       emit_insn (gen_ASH_right (operands[1], operands[1], GEN_INT (-2)));
     emit_move_insn (operands[0],
		     gen_rtx_UNSPEC (Pmode, gen_rtvec (1, x), UNSPEC_ADDRESS));
     emit_insn (pdp10_gen_stack_adjust (operands[1]));
     DONE;
   }")
