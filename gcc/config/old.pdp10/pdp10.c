...
/* Candidates for inlining once the code settles:
   ...
   stack_dynamic_offset, initial_frame_pointer_offset,
   output_func_prologue, output_func_epilogue, function_profiler,
   legitimate_address_p, legitimize_address, pdp10_const_int_costs,
...
int pdp10_argregs = PDP10_DEFAULT_REGPARM;
...
#define TARGET_ASM_OPEN_PAREN ASM_OPEN_PAREN

#undef TARGET_ASM_CLOSE_PAREN
#define TARGET_ASM_CLOSE_PAREN ASM_CLOSE_PAREN
...
/* Receives a pointer to the value of the -mpsect option.  */
char *pdp10_psect = 0;

/* Receives a pointer to the value of the -mmodel option.  */
char *pdp10_model = 0;

/* Receives a pointer to the value of the -margument-regs option.  */
     ...
     char *name;
{
  ...
  *q = 0;
  ...
  for (i = 0; i < lengthof (cpu_alias); i++)
    {
...
/* Return the number of registers available for -marguments-regs,
...
pdp10_override_options ()
{
  ...
  if (pdp10_arch || pdp10_tune)
    {
      for (i = 0; i < lengthof (cpu_table); i++)
	{
  ...
  if ((target_flags & OPT_EXTENDED)
    ...
    fprintf (stderr, "Can't use -mextended and -munextended \
at the same time\n");
  ...
  if (TARGET_EXTENDED)
    {
      if (!pdp10_psect)
	pdp10_psect = (char *)".text";
      pdp10_text_section_asm_op = xmalloc (15 + strlen (pdp10_psect));
  ...
  if (pdp10_argument_regs)
    {
      ...
      pdp10_argregs = atoi (pdp10_argument_regs);
      if (pdp10_argregs < 0 || pdp10_argregs > available_regs () - 1)
	{
	  fprintf (stderr, "Bad value for -margument-regs: %d\n",
		   pdp10_argregs);
	  exit (1);
	}

      for (i = 0; i < pdp10_argregs; i++)
	reg_alloc_order[i] = pdp10_argregs - i;
    }
...
/* Called by the INITIAL_FRAME_POINTER_OFFSET macro.  */
int
pdp10_initial_frame_pointer_offset ()
{
     ...
     HOST_WIDE_INT frame_size ATTRIBUTE_UNUSED;
{
  ...
  fprintf (stream, "\t%s current_function_args_size = %d\n",
	   ASM_COMMENT_START, current_function_args_size);
  fprintf (stream, "\t%s current_function_pretend_args_size = %d\n",
	   ASM_COMMENT_START, current_function_pretend_args_size);
  fprintf (stream, "\t%s frame_size = %d\n",
	   ASM_COMMENT_START, frame_size);
  fprintf (stream, "\t%s current_function_outgoing_args_size = %d\n",
	   ASM_COMMENT_START, current_function_outgoing_args_size);
  fprintf (stream, "\t%s frame_pointer_needed = %d\n",
  ...
  for (i = 0; i < FIRST_PSEUDO_REGISTER; i++)
      ...
      if (clobbered[i])
	{
	  ...
	  fputs (" callee-saved accumulator ");
	  print_reg (stream, i);
	  fputs (" clobbered\n", stream);
	}
    }
     ...
     int strict;
{
  ...
  switch (GET_CODE (x))
	...
	&& (REG_P (x)
	    || (GET_CODE (x) == MEM
		&& pdp10_legitimate_address_p (Pmode, XEXP (x, 0), strict)));
     ...
     rtx label;
{
  ...
  prev_insn = prev_active_insn (label);
  if (get_attr_skip (prev_insn) == SKIP_NO
      /* TODO: is any_condjump_p enough? */
      && !any_condjump_p (prev_insn))
    {
      rtx x;
      for (x = NEXT_INSN (label); GET_CODE (x) != BARRIER;
	   x = NEXT_INSN (x))
	if (GET_CODE (x) != NOTE)
	  INSN_DELETED_P (x) = 1;
    }
     ...
     int code;
{
  ...
  switch (code)
	...
	if (code == 'D')
	  fputs ("EXP ", stream);
	if (code != 'B')
	  pdp10_print_integer (stream, i >> 36);
	if (code == 'D')
	  fputc (',', stream);
	if (code != 'A')
	  pdp10_print_integer (stream, i & WORD_MASK);
      }
      return;
  ...
  switch (GET_CODE (x))
      ...
      if (GET_MODE (x) == VOIDmode)
	{
	  ...
	  if (code != 'A' && code != 'B')
	    fputs ("EXP ", stream);
	  if (code != 'B')
	    pdp10_print_integer (stream, (high << 28) + (low >> 36));
	  if (code != 'A' && code != 'B')
	    fputc (',', stream);
	  if (code != 'A')
	    pdp10_print_integer (stream, low & WORD_MASK);
	}
	...
	if (ps != 0 && code != 'W')
	  {
	    decode_ps (ps, &position, &bytesize);
...
static tree char6_signed_type_node;
static tree char6_unsigned_type_node;
static tree char7_signed_type_node;
       tree char7_unsigned_type_node;
static tree char8_signed_type_node;
static tree char8_unsigned_type_node;
static tree char9_signed_type_node;
static tree char9_unsigned_type_node;
static tree short16_integer_type_node;
static tree short16_unsigned_type_node;
static tree int32_integer_type_node;
static tree int32_unsigned_type_node;
...
pdp10_build_machine_types ()
{
  ...
  if (pdp10_char_bytesize)
    {
      int n = atoi (pdp10_char_bytesize);
      ...
      if (n != 9)
	{
	  ...
	  make_byte_type (char_type_node, n, !flag_signed_char);
	  signed_char_type_node = make_node (INTEGER_TYPE);
	  make_byte_type (signed_char_type_node, n, 0);
	  unsigned_char_type_node = make_node (INTEGER_TYPE);
	  make_byte_type (unsigned_char_type_node, n, 1);
	}
    }
...
/* Called by the MACHINE_DEPENDENT_REORG macro.
   ...
   Run as the last optimization pass.  (If there was a delayed branch
   scheduling pass in the PDP-10 compiler, it would run after this
   pass.)  */
void
	  ...
	  for (links = LOG_LINKS (insn); links; links = XEXP (links, 1))
	    {
	      ...
	      rtx insn_set_src = SET_SRC (PATTERN (insn));

	      if (GET_CODE (prev) == INSN
		  && get_attr_reorg_type (prev) == REORG_TYPE_IBP
		  && (GET_CODE (insn_set_src) == ZERO_EXTEND
		      || GET_CODE (insn_set_src) == SIGN_EXTEND)
		  && GET_CODE (XEXP (insn_set_src, 0)) == MEM
		  && SET_DEST (PATTERN (prev))
		     == XEXP (XEXP (insn_set_src, 0), 0))
		{
		  rtx pre_inc = gen_rtx_PRE_INC (QImode,
						 SET_DEST (PATTERN (prev)));
		  rtx mem = gen_rtx_MEM (QImode, pre_inc);
		  rtx x
		    = GET_CODE (insn_set_src) == ZERO_EXTEND
		    ? gen_rtx_ZERO_EXTEND (SImode, mem)
		    : gen_rtx_SIGN_EXTEND (SImode, mem);
					   
		  PATTERN (insn)
		    = gen_rtx_SET (VOIDmode, SET_DEST (PATTERN (insn)), x);
		  ...
		  && SET_DEST (PATTERN (prev))
		     == XEXP (SET_DEST (PATTERN (insn)), 0))
		{
		  rtx pre_inc = gen_rtx_PRE_INC (QImode,
						 SET_DEST (PATTERN (prev)));
		  rtx mem = gen_rtx_MEM (QImode, pre_inc);
					   
		  SET_DEST (PATTERN (insn)) = mem;
		  INSN_DELETED_P (prev) = 1;
		}
	    }
	}
...
/* Return nonzero if OP is a memory operand with a variable address.  */
int
pdp10_varaddr_memory_operand (op, mode)
     rtx op;
     enum machine_mode mode;
{
     ...
     rtx nextarg;
{
  ...
  if (current_function_varargs)
    /* When current_function_varargs goes away, remember that stdard_p
       is always true.  */
    ;
...
pdp10_output_byte (stream, exp, byte_size)
     ...
     int byte_size;
{
  if (last_output_byte == NULL)
    {
      if (exp == NULL)
	return;
      fprintf (stream, "\tBYTE (%d)", byte_size);
      pdp10_print_integer (stream, INTVAL (exp));
     ...
     int aligned_p;
{
  ...
  switch (size)
    ...
    case 1:
    case 2:
      pdp10_output_byte (asm_out_file, x, BITS_PER_UNIT * size);
      break;
    case 4:
    case 8:
...
#if 0
      {
	tree type = lookup_symbol_type (XSTR (x, 0));
	return AGGREGATE_TYPE_P (type) ? 0 : 4 - int_size_in_bytes (type);
      }
#endif
    case LABEL_REF:
    case REG:
     ...
     rtx *operands;
{
  ...
  if (GET_CODE (operands[0]) == MEM
      && MEM_ALIGN (operands[0]) == BITS_PER_WORD)
    pos = 0;
...
#else
      if (GET_CODE (x) == MEM && !MEM_SCALAR_P (x))
	mem_right = pdp10_pointer_offset (XEXP (x, 0)) % UNITS_PER_WORD != 0;
#endif
...
#define INTVAL_OR_ELSE(X, Y) (GET_CODE (X) == CONST_INT ? INTVAL (X) : (Y))
...
sign_extended_p (rtx x, int bits, rtx insn)
{
  ...
  do
	...
	{
	case ASHIFTRT:
	  n = INTVAL_OR_ELSE (XEXP (x, 1),
			      GET_CODE (XEXP (x, 1)) == NEG ?
			      -INTVAL_OR_ELSE (XEXP (XEXP (x, 1), 0), 0) : 0);
	  x = XEXP (x, 0);
	  break;
	case SIGN_EXTRACT:
	  n = BITS_PER_WORD - INTVAL_OR_ELSE (XEXP (x, 1), BITS_PER_WORD);
...
#if REAL_BITS_PER_UNIT == 9
  if (GET_CODE (x) == MEM && pos != -1)
    normalize_bitfield_mem (&x, &pos);
#endif
...
#if REAL_BITS_PER_UNIT == 9
  normalize_bitfield_mem (&operands[1], &pos);
#endif
#if 0
  rtx op1 = operands[1];
  if (pos != 0
      ...
      && (!TARGET_EXTENDED
	  || TARGET_SMALLISH
	  || (VALID_OWGBP_BYTE_SIZE_P (len)
	      && CONSTANT_ADDRESS_P (XEXP (operands[1], 0)))))
    {
      output_extv++;
      operands[1] = op1;
      pdp10_output_extzv (insn, operands);
      operands[2] = gen_int_mode (HWINT (1) << (len - 1), SImode);
      operands[3] = gen_int_mode ((HWINT (1) << len) - 1, SImode);
      return "trne %0,%2\n\t%_orcmi %0,%3";
    }
  else
#endif
      ...
      if (len == 18)
	{
	  if (pos == 0)
	    return "hlre %0,%W1";
	  if (pos == 18)
	    return REG_P (operands[1]) && REGNO (operands[1]) != 0
	      ? "hrrei %0,(%1)" : "hrre %0,%W1";
	}
...
#else
pdp10_expand_movstrqi (operands)
#endif
     rtx *operands;
{
     ...
     rtx length;
{
  ...
  acs = gen_reg_rtx (TImode);
  emit_move_insn (gen_rtx_SUBREG (SImode, acs, 0),
		  GEN_INT (INTVAL (length) / UNITS_PER_WORD - 1));
...
#else
pdp10_expand_clrstrqi (operands)
#endif
     rtx *operands;
{
  /* operands[0] is the block destination address
...
pdp10_expand_addbp3 (type, target, op0, op1)
     ...
     rtx op1;
{
  /* First, try to use IBP.  */
  if (GET_CODE (op1) == CONST_INT
      && maybe_expand_ibp (pdp10_bytesize (type), target, op0, INTVAL (op1)))
    return target;
     ...
     int add;
{
  ...
  if (WORD_TYPE_P (type))
	...
	{
	  increment = gen_reg_rtx (Pmode);
	  emit_insn (gen_ashrsi3_pointer (increment, op1, GEN_INT (2)));
	}
#else
      if (GET_CODE (op1) == CONST_INT)
	{
	  ...
	  if (CONSTANT_ADDRESS_P (op0))
	    {
	      emit_move_insn (target, plus_constant (op0, i));
	      if (neg)
...
#if 0
      int size = pdp10_bytesize (type);
      if (size == -1 && TREE_CODE (type) == VOID_TYPE)
	size = 9;
	...
	case 18:
	  if (GET_CODE (op1) == CONST_INT)
	    increment = GEN_INT (INTVAL (op1) / 2);
	  else
	    {
	      increment = gen_reg_rtx (Pmode);
	      emit_insn (gen_ashrsi3_pointer (increment, op1, GEN_INT (1)));
	    }
	  break;
      ...
      if (!add)
	{
	  if (GET_CODE (increment) == CONST_INT)
	    ...
	    emit_insn (gen_negsi2 (increment, increment));
	}
     ...
     tree type1 ATTRIBUTE_UNUSED;
{
  ...
  if (WORD_TYPE_P (type))
      ...
      else
	{
	  ...
	  target = gen_rtx_SUBREG (SImode, temp, 4);
	  ...
	  emit_insn (gen_ashrsi3 (target, target, GEN_INT (shift)));
	  ...
	  emit_insn (gen_addsi3 (target, target, mem));

	  if (bytesize == 18)
	    emit_insn (gen_ashlsi3_pointer (target, target, GEN_INT (1)));
	}
    }
...
/* Return or output a single instruction to add to a word pointer.  */
const char *
pdp10_output_addwp3 (destination, source)
     rtx destination;
     rtx source;
{
  const char *insn;
  ...
  if (GET_CODE (destination) == MEM)
	...
	{
	  immediate = 1;
	  insn = INTVAL (source) >= 0 ? "addi" : "subi";
	}
      else
	insn = "add";
      op0 = destination;
  ...
  fputc ('\t', asm_out_file);
  pdp10_print_insn (asm_out_file, insn);
  pdp10_print_tab (asm_out_file);
  pdp10_print_operand (asm_out_file, op0, 0);
  fputc (',', asm_out_file);
  if (immediate)
    pdp10_print_offset30 (asm_out_file, labs (INTVAL (op1)) & ADDRESS_MASK);
  else
    pdp10_print_operand (asm_out_file, op1, 0);
  fputc ('\n', asm_out_file);
...
static void	expand_save_or_restore_return_address PARAMS ((int save,
							       int clob_size));
#if 0 /* Remove me.  */
static void	expand_save_pretend_args PARAMS ((int clobbered_size));
#endif
...
#if 0 /* Remove me.  */
/* If necessary, emit instructions to store register arguments in the
   stack.  */
static void
expand_save_pretend_args (clobbered_size)
     int clobbered_size;
{
  int offset = -((current_function_outgoing_args_size +
		 ...
		 / UNITS_PER_WORD);
  tree type = TYPE_ARG_TYPES (TREE_TYPE (cfun->decl));
  int i, j;

  for (i = 4; i > 4 - current_function_pretend_args_size / UNITS_PER_WORD; i--)
    {
      emit_move_insn (GEN_MEM_SP_PLUS (SImode, offset),
     ...
     int single;
{
  ...
  for (i = 0; i < FIRST_PSEUDO_REGISTER; i += regs)
    {
      regs = 1;
      if (!clobbered[i])
	continue;
...
expand_set_up_frame_pointer (void)
{
  ...
  INITIAL_ELIMINATION_OFFSET (FRAME_POINTER_REGNUM, STACK_POINTER_REGNUM,
			      offset);
  emit_frame_insn (gen_MOVEI (frame_pointer_rtx,
...
static tree	strip_conversions PARAMS ((tree exp));
     ...
     tree exp;
{
  ...
  /* GCC parses a pointer subtraction like this:
	...
	n = x - y;
     as:
	n = (int)x - (int)y;
     Therefore, we need to strip away the conversions from the
     operands in order to see their real types.  */

#if 1
  exp0 = strip_conversions (TREE_OPERAND (exp, 0));
#else
  ...
  if (TREE_CODE_CLASS (code) == '<' || TREE_CODE_CLASS (code) == '2')
    {
#if 1
      exp1 = strip_conversions (TREE_OPERAND (exp, 1));
#else
  ...
  switch (code)
	...
	return expand_subtract_pointers (mode, target, op0, type0, op1, type1);
      if (ptr0 || ptr1)
	return expand_add_or_sub_pointer (target, op0, type0, op1, type1, add);
      break;
...
rtx
pdp10_convert_pointer (exp, op0, target, mod, stack)
     tree exp;
     rtx op0;
     rtx target ATTRIBUTE_UNUSED;
     int mod;
...
rtx
pdp10_convert_ptr (exp, op0, target, from, to)
     tree exp;
     rtx op0;
     rtx target ATTRIBUTE_UNUSED;
     int from;
     int to;
{
  ...
  if (/* exp && TREE_CONSTANT (exp) || */
      GET_CODE (op0) == CONST || GET_CODE (op0) == CONST_INT)
    {
      HOST_WIDE_INT offset = 0, ps;
      rtx x = op0;
  ...
  emit_jump = !(to == 36 || (to == 9 && from == 18 && TARGET_EXTENDED));
  if (emit_jump)
    emit_jump_insn (gen_cbranchsi (label, op0, GEN_INT (0), GEN_EQ));
     ...
     int mod;
{
  ...
  /* If we are taking the address of something erroneous, just
	...
	{
#if 0
	  if (TREE_CODE (TREE_OPERAND (exp, 0)) == STRING_CST)
	    {
	      HOST_WIDE_INT ps = TARGET_EXTENDED ? 07000 : 03311;
	      op0 = plus_constant (op0, ps << 24);
	    }
#endif
	  return op0;
	}
  ...
  if (GET_CODE (op0) != LABEL_REF)
    {
      rtx temp = pdp10_convert_pointer (exp, op0, NULL_RTX, EXPAND_NORMAL, 1);
      if (temp)
	op0 = temp;
    }
