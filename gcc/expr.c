     ...
     int unsignedp;
{
  ...
  if (to_real != from_real) /* redundant? */
    abort ();
     ...
     unsigned int align;
{
  ...
  if (! SLOW_UNALIGNED_ACCESS (word_mode, align)
      || align > (unsigned int) (MOVE_MAX * BITS_PER_UNIT)
     ...
     rtx size;
{
  ...
  if (GET_CODE (size) == CONST_INT && MOVE_BY_PIECES_P (INTVAL (size), align))
	   ...
	   mode = GET_MODE_WIDER_MODE (mode))
	{
	 ...
	 byte pointers, as used to represent the "void *" type.  */
      {
	int x_bitsize, y_bitsize;
	...
	if (XEXP (x, 0) == virtual_stack_vars_rtx
	  ...
	  x_bitsize = pdp10_bytesize (TREE_TYPE (MEM_EXPR (x)));
	else
	  x_bitsize = MEM_ALIGN (x);

	if (XEXP (y, 0) == virtual_stack_vars_rtx
	    || (GET_CODE (XEXP (y, 0)) == PLUS
		&& XEXP (XEXP (y, 0), 0) == virtual_stack_vars_rtx))
	  y_bitsize = 36;
	else if (XEXP (y, 0) == virtual_outgoing_args_rtx
		 || (GET_CODE (XEXP (y, 0)) == PLUS
		     && XEXP (XEXP (y, 0), 0) == virtual_outgoing_args_rtx))
	  y_bitsize = 36;
	else if (MEM_EXPR (y))
	  y_bitsize = pdp10_bytesize (TREE_TYPE (MEM_EXPR (y)));
	else
	  y_bitsize = MEM_ALIGN (y);

        x = pdp10_convert_ptr (NULL_TREE, XEXP (x, 0), NULL_RTX,
			       x_bitsize, 9, 1);
	y = pdp10_convert_ptr (NULL_TREE, XEXP (y, 0), NULL_RTX,
			       y_bitsize, 9, 1);
      }
#else
...
/* Generate code to push X onto the stack, assuming it has mode MODE and
   ...
   If PARTIAL and REG are both nonzero, then copy that many of the first<-nope
     ...
     rtx alignment_pad;
{
  ...
  if (mode == BLKmode)
	...
	abort ();
	 ...
	 because registers will take care of them.  */
      ...
      if (partial != 0 && stack_direction == downward)
	xinner = adjust_address (xinner, BLKmode,
				 used / (UNITS_PER_WORD / REAL_UNITS_PER_WORD));
	  ...
	  if (! args_addr)
	    ...
	    temp = memory_address (BLKmode,
				   plus_constant (args_addr,
						  (skip + INTVAL (args_so_far)) / (UNITS_PER_WORD / REAL_UNITS_PER_WORD)));
	  else
	    /* FIXME: need to divide args_so_far by UNITS_PER_WORD.  */
	    temp = memory_address (BLKmode,
 ...
 ret:
  ...
  if (partial > 0 && reg != 0)
    {
#ifndef STACK_GROWS_DOWNWARD
      /* It the stack grows downward, copy the from the end.  */
      if (size == NULL_RTX || GET_CODE (size) == CONST_INT)
	{
	  if (GET_CODE (x) == MEM)
	    x = adjust_address (x, GET_MODE (x),
				size
				? INTVAL (size) / UNITS_PER_WORD
				: GET_MODE_SIZE (mode));
	  else
	    {
	      if (GET_MODE_SIZE (mode) == 8)
		{
		  x = gen_lowpart (SImode, x);
		  mode = SImode;
		}
	      else if (GET_MODE_SIZE (mode) == 16 && GET_CODE (x) == CONCAT)
		{
		  if (partial == 3)
		    x = gen_rtx_PARALLEL (VOIDmode,
					  gen_rtvec (2,
						     gen_lowpart (SImode,
								  XEXP (x, 0)),
						     XEXP (x, 1)));
	...
	{
	  rtx tmp = expand_binop (Pmode, udiv_optab, tmp,
				  GEN_INT (UNITS_PER_WORD),
				  NULL_RTX, 0, OPTAB_LIB_WIDEN);
	  x = change_address (x, VOIDmode,
			      expand_binop (Pmode, add_optab, XEXP (x, 0), tmp,
					    NULL_RTX, 0, OPTAB_LIB_WIDEN));
	}
#endif
...
static rtx single_component_in_word_p PARAMS ((tree to, rtx to_rtx,
					       HOST_WIDE_INT *pbitsize));

static rtx
single_component_in_word_p (to, to_rtx, pbitpos)
     tree to;
     rtx to_rtx;
     HOST_WIDE_INT *pbitpos;
{
  ...
  field = TREE_OPERAND (to, 1);

  offset = tree_low_cst (DECL_FIELD_OFFSET (field), 1);
#if REAL_UNITS_PER_WORD == UNITS_PER_WORD
  if (offset % UNITS_PER_WORD != 0
      || tree_low_cst (DECL_FIELD_BIT_OFFSET (field), 1) != 0
      || tree_low_cst (DECL_SIZE (field), 1) >= BITS_PER_WORD)
    return NULL_RTX;
#endif

  for (f = TYPE_FIELDS (DECL_FIELD_CONTEXT (field)); f != NULL_TREE;
       f = TREE_CHAIN (f))
    {
      if (offset > (tree_low_cst (DECL_FIELD_OFFSET (f), 1)
		    + tree_low_cst (DECL_SIZE_UNIT (f), 1))
	  || ! DECL_NAME (f)
	  || f == field)
	continue;
      if (tree_low_cst (DECL_FIELD_OFFSET (f), 1)
	  >= offset + REAL_UNITS_PER_WORD)
	break;
      return NULL_RTX;
    }

  offset = UNITS_PER_WORD * (*pbitpos / BITS_PER_WORD);
  *pbitpos %= REAL_BITS_PER_UNIT;

  {
#if REAL_BITS_PER_UNIT == BITS_PER_WORD
    HOST_WIDE_INT mask = ~(HOST_WIDE_INT)0;
#else
    HOST_WIDE_INT mask = ~(HOST_WIDE_INT)(UNITS_PER_WORD - 1);
#endif

    if (GET_CODE (to_rtx) == REG)
      return gen_rtx_SUBREG (SImode, to_rtx, offset & mask);
    else
      {
	offset /= UNITS_PER_WORD / REAL_UNITS_PER_WORD;
	return adjust_address (to_rtx, SImode, offset & mask);
      }
  }
}
     ...
     int suggest_reg ATTRIBUTE_UNUSED;
{
  ...
  if (TREE_CODE (to) == COMPONENT_REF || TREE_CODE (to) == BIT_FIELD_REF
	  ...
	  if (GET_CODE (to_rtx) == MEM
	      ...
	      && MEM_ALIGN (to_rtx) == GET_MODE_ALIGNMENT (mode1))
	    {
	      to_rtx = adjust_address (to_rtx, mode1,
				       /* PDP-10: adjust by number of
                                          words.  */
				       bitpos / REAL_BITS_PER_UNIT);
	      bitpos = 0;
	    }

	  if (bitsize < 32
	      && GET_CODE (offset_rtx) == MULT
	      && GET_CODE (XEXP (offset_rtx, 1)) == CONST_INT
	      && ((INTVAL (XEXP (offset_rtx, 1)) * bitsize)
		  % BITS_PER_WORD) == 0)
	    {
	      rtx var = XEXP (offset_rtx, 0);
	      HOST_WIDE_INT cst = INTVAL (XEXP (offset_rtx, 1));

	      cst /= BITS_PER_WORD / bitsize;
	      if (cst == 1)
		offset_rtx = var;
	      else
		offset_rtx = gen_rtx_MULT (Pmode, var, GEN_INT (cst));

	      to_rtx = offset_address (to_rtx, offset_rtx,
				       highest_pow2_factor (offset));
	    }
	  else if (GET_MODE_SIZE (mode1) < UNITS_PER_WORD)
	    {
	      rtx temp = gen_reg_rtx (Pmode);
	      rtx addr;

	      if (GET_CODE (offset_rtx) == QUEUED)
		offset_rtx = force_reg (Pmode, offset_rtx);

	      if (GET_CODE (to_rtx) == MEM)
		addr = force_reg (Pmode, XEXP (to_rtx, 0));
	      else if (GET_CODE (to_rtx) == SUBREG
		       && GET_CODE (SUBREG_REG (to_rtx)) == ZERO_EXTRACT
		       && GET_CODE (XEXP (SUBREG_REG (to_rtx), 0)) == MEM)
		addr = force_reg (Pmode,
				  XEXP (XEXP (SUBREG_REG (to_rtx), 0), 0));
	      else
		abort ();

	      /* FIXME: is this a special case?  */
	      if ((REG_P (XEXP (to_rtx, 0))
		   && XEXP (to_rtx, 0) == virtual_stack_vars_rtx)
		  || (GET_CODE (XEXP (to_rtx, 0)) == PLUS
		      && REG_P (XEXP (XEXP (to_rtx, 0), 0))
		      && XEXP (XEXP (to_rtx, 0), 0) == virtual_stack_vars_rtx))
		{
		  /* FIXME: code duplication.  */
		  HOST_WIDE_INT ps;
		  if (! TARGET_EXTENDED)
		    abort ();
		  if ((mode1 == VOIDmode && bitsize <= 9)
		  ...
		  emit_insn (gen_iorsi3 (addr,
					 force_reg (Pmode, XEXP (to_rtx, 0)),
					 GEN_INT (trunc_int_for_mode
						  (ps << 30, Pmode))));
		}
	      else if (GET_CODE (to_rtx) == MEM)
		addr = XEXP (to_rtx, 0);
	      else
	      ...
	      if (offset_rtx != const0_rtx)
		{
		  ...
		  if (pdp10_bytesize (TREE_TYPE (tem)) >= BITS_PER_WORD)
	...
	rtx word = single_component_in_word_p (to, to_rtx, &bitpos1);
	if (word && bitsize < BITS_PER_WORD
	    && TREE_CODE (TREE_TYPE (from)) == INTEGER_TYPE)
	  {
	    tree amount = build_int_2 (BITS_PER_WORD - bitsize - bitpos1, 0);
	    rtx from_rtx, temp;

	    from_rtx = expand_expr (from, NULL_RTX, SImode, 0);
	    temp = expand_shift (LSHIFT_EXPR, SImode, from_rtx, amount,
				 NULL_RTX, 1);
	    emit_move_insn (word, temp);
	    if (want_value)
	      {
		/* The caller wants an rtx for the value.  If
		   possible, avoid refetching from the bitfield
		   itself.  */
		if (GET_CODE (to_rtx) != MEM || ! MEM_VOLATILE_P (to_rtx))
		  {
		    tree count;
			...
			rtx mask
			  = GEN_INT (trunc_int_for_mode
				     (((HOST_WIDE_INT) 1 << bitsize) - 1,
				      SImode));
			result = expand_and (SImode, from_rtx, mask, NULL_RTX);
		      }
		    else
		      {
			count = build_int_2 (BITS_PER_WORD - bitsize, 0);
			temp = expand_shift (LSHIFT_EXPR, SImode, from_rtx,
					     count, 0, 0);
			result = expand_shift (RSHIFT_EXPR, SImode, temp,
					       count, 0, 0);
		      }
		    if (mode1 != VOIDmode)
		      result = convert_to_mode (mode1, result, unsignedp);
		  }
		else
		  result = extract_bit_field (orig_to_rtx, bitsize, bitpos1,
					      unsignedp, NULL_RTX,
					      mode1 != VOIDmode ? mode1 : SImode,
					      VOIDmode,
					      int_size_in_bytes
					      (TREE_TYPE (tem)));
	      }
  ...
  if (TREE_CODE (to) == RESULT_DECL && TREE_CODE (from) == INDIRECT_REF
	...
	from_addr = pdp10_convert_ptr (NULL_TREE, from_addr, NULL_RTX,
      ...
      emit_library_call (memmove_libfunc, LCT_NORMAL,
			 VOIDmode, 3, to_addr, Pmode, from_addr, Pmode,
			 convert_to_mode (TYPE_MODE (sizetype),
					  size, TREE_UNSIGNED (sizetype)),
			 TYPE_MODE (sizetype));
#else
      emit_library_call (bcopy_libfunc, LCT_NORMAL,
			 VOIDmode, 3, from_addr, Pmode, to_addr, Pmode,
			 convert_to_mode (TYPE_MODE (integer_type_node),
     ...
     int alias_set;
{
  ...
  if (mode == VOIDmode
      ...
      if (GET_MODE (target) == BLKmode && GET_MODE (temp) == BLKmode)
	{
	  ...
	  /* PDP-10: adjust by number of words.  */
	  target = adjust_address (target, VOIDmode,
				   bitpos / REAL_BITS_PER_UNIT);
	...
	to_rtx = replace_equiv_address (to_rtx, copy_to_reg (addr));
      ...
      to_rtx = adjust_address (target, mode, bitpos / BITS_PER_UNIT
			       /* PDP-10.  */
			       / (UNITS_PER_WORD / REAL_UNITS_PER_WORD));
     ...
     int *pvolatilep;
{
  ...
  if (TREE_CODE (exp) == COMPONENT_REF)
	...
	*pbitsize = tree_low_cst (TYPE_SIZE (TREE_TYPE (exp)), 1); /*GET_MODE_BITSIZE (mode);*/
    }
  ...
  while (1)
	       ...
	       || TREE_CODE (exp) == ARRAY_RANGE_REF)
	{
	  ...
	  /* PDP-10: If accessing data smaller than an array item
	     (say, in a struct member in an array of structs), and the
	     data is smaller than a word, make offset count in number
	     of bytes.  */
	  if (TREE_CONSTANT (unit_size)
	      && *pbitsize < 32
	      && 9 * tree_low_cst (unit_size, 1) > *pbitsize)
	    {
	      int n = 9 * tree_low_cst (unit_size, 1) / *pbitsize;
	      offset = size_binop (MULT_EXPR, offset,
				   convert (sizetype, build_int_2 (n, 0)));
	    }
	}
...
#endif

static tree hejhopp PARAMS ((tree exp));
static tree
hejhopp (exp)
     tree exp;
{
...
static int blah PARAMS ((int ps, int from, int to));
static int
blah (ps, from, to)
     int ps;
     int from;
     int to;
{
  ...
  if (TARGET_EXTENDED)
    {
      static int gb9[] =  { 07000, 07100, 07200, 07300 };
      static int gb18[] = { 07500, 07500, 07600, 07600 };
    ...
    {
      static int lb9[] =  { 03311, 02211, 01111, 00011 };
      static int lb18[] = { 02222, 02222, 00022, 00022 };
     ...
     enum expand_modifier modifier;
{
  ...
  switch (code)
    ...
    case INTEGER_CST:
      /* PDP-10: take the 71-bit DImode format into account.  */
      if (TARGET_71BIT
	      ...
	      || TYPE_MAIN_VARIANT (type) == long_long_unsigned_type_node))
	{
	  ...
	  HOST_WIDE_INT mask = ((HOST_WIDE_INT)1 << 35) - 1;
	  i1 = (i1 << 1) + (i0 < 0);
	  if (i1 >> (72 - HOST_BITS_PER_WIDE_INT) != 0
	      && i1 >> (72 - HOST_BITS_PER_WIDE_INT) != -1)
	    {
	      if (i1 >= 0)
		i1 &= ((HOST_WIDE_INT)1 << (72 - HOST_BITS_PER_WIDE_INT)) - 1;
	      else
		{
		  i1 <<= 72 - HOST_BITS_PER_WIDE_INT;
		  i1 >>= 72 - HOST_BITS_PER_WIDE_INT;
		}
	    }
	  i0 = (i0 & mask) + ((i0 & ~mask) << 1);
	  if (i1 & ((HOST_WIDE_INT)1 << (71 - HOST_BITS_PER_WIDE_INT)))
	...
	if (offset != 0)
	  {
	    ...
	    /* If the offset is an integral multiple of words, don't
	       use ADJBP.  */
	    else if (GET_CODE (offset_rtx) == MULT
		     && GET_CODE (XEXP (offset_rtx, 1)) == CONST_INT
		     && (pdp10_bytesize (TREE_TYPE (exp))
			 * INTVAL (XEXP (offset_rtx, 1))) % 36 == 0)
	      {
		rtx var = XEXP (offset_rtx, 0);
		HOST_WIDE_INT cst = INTVAL (XEXP (offset_rtx, 1));

		cst /= BITS_PER_WORD / pdp10_bytesize (TREE_TYPE (exp));
		if (cst == 1)
		  offset_rtx = var;
		else
		  offset_rtx = gen_rtx_MULT (Pmode, var, GEN_INT (cst));
		...
		addr = XEXP (mem, 0);
		/* FIXME: special case.  */
		if (addr == virtual_stack_vars_rtx
		    || (GET_CODE (addr) == PLUS
			&& XEXP (addr, 0) == virtual_stack_vars_rtx))
		  {
		    int bytesize = pdp10_bytesize (TREE_TYPE (exp));
		    int ps;
		    ...
		    addr = expand_binop (Pmode, ior_optab, addr,
					 GEN_INT (trunc_int_for_mode
						  ((HOST_WIDE_INT) ps << 30,
						   Pmode)),
					 NULL_RTX, 1, 0);
		...
		if (pdp10_bytesize (TREE_TYPE (tem)) >= BITS_PER_WORD)
		  addr = pdp10_convert_ptr (NULL_TREE, addr, NULL_RTX, 36,
					    pdp10_bytesize (TREE_TYPE (exp)),
					    GET_CODE (offset_rtx) != CONST_INT
					    || INTVAL (offset_rtx) != 0);

		temp = gen_reg_rtx (Pmode);
		/* temp = pdp10_expand_addbp3 (type, temp, addr,
		   offset_rtx); */
		emit_insn (gen_ADJBP (temp, force_reg (Pmode, addr),
	...
	if (modifier == EXPAND_CONST_ADDRESS
	  ...
	  op0 = adjust_address_nv (op0, mode1, bitpos / REAL_BITS_PER_UNIT);
	else
	  op0 = adjust_address (op0, mode1, bitpos / REAL_BITS_PER_UNIT);
#else
	/* PDP-10.  */
	{
	  rtx old_op0 = op0;
	  if ((code != ARRAY_REF
		...
		op0 = adjust_address (op0, mode2, bitpos / REAL_BITS_PER_UNIT);
...
#ifdef MD_CONVERT_POINTER
      if ((POINTER_TYPE_P (type)
	   || POINTER_TYPE_P (TREE_TYPE (TREE_OPERAND (exp, 0))))
	  && type != TREE_TYPE (TREE_OPERAND (exp, 0)))
	{
	  int not_null = TREE_CODE (TREE_OPERAND (exp, 0)) == PLUS_EXPR;
	  rtx x = MD_CONVERT_POINTER (exp, target, modifier, not_null);
	  if (x)
	    return x;
	}
#endif
	  ...
	  (TYPE_SIZE (TREE_TYPE (TREE_OPERAND (exp, 0))), 1) % 9 != 0)
	{
	  ...
	  else if (TREE_UNSIGNED (TREE_TYPE (TREE_OPERAND (exp, 0))))
	  ...
	  && CONSTANT_P (XEXP (op1, 1)))
	{
      ...
      if (POINTER_TYPE_P (type)
	  && pdp10_bytesize (TREE_TYPE (type)) < BITS_PER_WORD)
	{
	  rtx old, new, tmp;
	  tree from_type = TREE_TYPE (hejhopp (TREE_OPERAND (exp, 0)));
	  int from, to = pdp10_bytesize (TREE_TYPE (type));

	  if (GET_CODE (op0) != REG
	      && POINTER_TYPE_P (from_type)
	      && (from = pdp10_bytesize (TREE_TYPE (from_type))) != to)
	    {
	      HOST_WIDE_INT offset = 0;
	      int ps = 0;
		...
		new = XEXP (new, 0);
	      if (GET_CODE (new) == PLUS)
		{
		  offset = INTVAL (XEXP (new, 1));
		  ps = (offset >> 24) & (TARGET_EXTENDED ? 07700 : 07777);
		  if (TARGET_EXTENDED)
		    offset &= ((HOST_WIDE_INT)1 << 30) - 1;
		  else
		    offset &= ((HOST_WIDE_INT)1 << 18) - 1;
		  new = XEXP (new, 0);
		}

	      ps = blah (ps, from, to);
	      op0 = plus_constant (new, offset + trunc_int_for_mode ((HOST_WIDE_INT)ps << 24, SImode));
	    }
	  ...
	  if (GET_CODE (tmp) != PLUS
	    ...
	    {
	      HOST_WIDE_INT ps;
	      int bytesize = pdp10_bytesize (TREE_TYPE (type));
	      if (TARGET_EXTENDED)
		ps =
		  bytesize == 6 ? 04600
		  : bytesize == 7 ? 06200
		...
		ps =
		  ...
		  : bytesize == 18 ? 02222
		  : ((void)abort (), 0);
	      old = plus_constant (op0, ps << 24);
	    }

	  if (modifier == EXPAND_INITIALIZER)
	    {
	      old = gen_rtx_ADJBP (Pmode, old, op1);
	      new = pdp10_simplify_unspec (old, NULL_RTX);
	      return new ? new : old;
	    }
	  else
	    {
	      temp = gen_reg_rtx (Pmode);
	      return pdp10_expand_addbp3 (TREE_TYPE (type), temp, old, op1);
	    }
	}
    ...
    case ADDR_EXPR:
#ifdef MD_EXPAND_ADDR_EXP
      return MD_EXPAND_ADDR_EXP (exp, type, target, ignore, modifier);
#else
  ...
  if (post)
    {
	 ...
	 the old value for subsequent use.  */
      ...
      /* PDP-10: use ADJBP to increment or decrement a byte pointer.  */
      if (POINTER_TYPE_P (TREE_TYPE (TREE_OPERAND (exp, 0)))
	  && pdp10_bytesize (TREE_TYPE (TREE_TYPE (TREE_OPERAND (exp, 0)))) < 32)
	icode = CODE_FOR_ADJBP;
      else
	icode = (int) this_optab->handlers[(int) mode].insn_code;
...
#ifdef MD_EXPAND_BINOP
  op1 = MD_EXPAND_BINOP (mode, this_optab, value, op1, op0,
...
#ifdef MD_COMPARE_AND_JUMP
  MD_COMPARE_AND_JUMP (exp, op0, op1, code, unsignedp, mode,
		       ((mode == BLKmode)
     ...
     int only_cheap;
{
  ...
  if ((code == NE || code == EQ)
	...
	subtarget = 0;
      ...
      /* PDP-10: do conversion from small sizes here.  */
      if (GET_MODE (op0) != mode && GET_MODE_SIZE (GET_MODE (op0)) < 4)
	op0 = convert_to_mode (mode, op0, ops_unsignedp);

      if (bitnum != 0)
	op0 = expand_shift (RSHIFT_EXPR,
			    /* PDP-10: use op0 mode instead of
			       operand_mode.  */
			    GET_MODE (op0),
			    op0, size_int (bitnum), subtarget, ops_unsignedp);
