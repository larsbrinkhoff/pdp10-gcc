     ...
     rtx insn;
{
  ...
  switch (code)
	...
	/* PDP-10: fold (mem:QI (const:SI <constant byte pointer>)) into
	   (zero_extend:SI (mem:SI ...)).  */
	if (base && CONSTANT_ADDRESS_P (base)
	    && GET_MODE_SIZE (mode) < UNITS_PER_WORD)
	  {
	    int ps = (offset >> 30) & 077;
	    int pos, size, b;
	    ...
	    pos = size * (ps - b);
	    return gen_rtx_ZERO_EXTRACT (SImode,
					 gen_rtx_MEM (SImode,
						      plus_constant (base,
								     offset)),
					 GEN_INT (size),
...
#ifdef MD_SIMPLIFY_UNSPEC
      else if (code == UNSPEC)
	new = MD_SIMPLIFY_UNSPEC (x, insn);
#endif
      break;
    }
