...
#ifdef __PDP10_H__
	  ASM_OUTPUT_DEBUG_LABEL (file, "", CODE_LABEL_NUMBER (insn));
#else
...
#ifdef __PDP10_H__
		ASM_OUTPUT_INTERNAL_LABEL (file, "", CODE_LABEL_NUMBER (insn));
#else
     ...
     rtx *xp;
{
  ...
  /* simplify_subreg does not remove subreg from volatile references.
     We are required to.  */
  if (GET_CODE (y) == MEM)
    *xp = adjust_address (y, GET_MODE (x),
			  /* PDP-10: word offset.  */
			  SUBREG_BYTE (x) / (UNITS_PER_WORD / REAL_UNITS_PER_WORD));
  else
    {
