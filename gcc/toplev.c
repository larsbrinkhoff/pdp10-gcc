...
debug_args[] =
{
  ...
  { "tops20", TOPS20_DEBUG, 0, N_("Generate TOPS-20 format debug info") },
#endif
  { 0, 0, 0, 0 }
};
...
#endif
#ifdef TOPS20_DEBUGGING_INFO
  if ((write_symbols == TOPS20_DEBUG) && toplev)
    dwarf2out_decl (TYPE_STUB_DECL (type));
#endif
  timevar_pop (TV_SYMOUT);
}
     ...
     tree decl;
{
...

  {
    /* PDP-10: change (set (subreg (zero_extract ...)) ...) to
       (set (zero_extract ...) ...).  */
    extern void pdp10_fixup_subreg (rtx);
    pdp10_fixup_subreg (insns);
  }
