...
{
  unsigned int unit
    = (GET_CODE (str_rtx) == MEM) ? REAL_BITS_PER_UNIT : BITS_PER_WORD;
  ...
  if (HAVE_insv
	...
	xop0 = adjust_address (xop0, word_mode, offset);
#else
     ...
     HOST_WIDE_INT total_size;
{
  ...
  if (bitsize > BITS_PER_WORD)
      ...
      if (unsignedp)
	{
      ...
      /* PDP-10: take the DImode integer format into account.  */
      if (mode == DImode && TARGET_71BIT)
	{
	  emit_insn (gen_LSHC_left (target, target, GEN_INT (2)));
	  emit_insn (gen_lshrsi3 (gen_rtx_SUBREG (SImode, target, 4),
				  gen_rtx_SUBREG (SImode, target, 4),
				  GEN_INT (1)));
	  emit_insn (gen_ASHC_right (target, target, GEN_INT (-1)));
		...
		&& (bitsize + bitpos > GET_MODE_BITSIZE (extzv_mode))))
	{
	  ...
	  if (GET_CODE (xop0) == MEM)
		     ...
		     (xop0, GET_MODE (xop0))))
		{
		  ...
		  xbitpos = bitnum % unit;
		  xop0 = adjust_address (xop0, bestmode, xoffset / 4);
  ...
  if (GET_CODE (op0) == SUBREG || GET_CODE (op0) == REG)
      ...
      if (bitpos >= total_bits)
	{
	 ...
	 Then alter OP0 to refer to that word.  */
      ...
      offset -= (offset % (total_bits / BITS_PER_UNIT));
      /* PDP-10: adjust by number of words.  */
      op0 = adjust_address (op0, mode, offset / 4);
    }
     ...
     int max_cost;
{
  ...
  if (moptab->handlers[(int) wider_mode].insn_code != CODE_FOR_nothing
      ...
      if (tem != 0)
	{
	  ...
	  /* PDP-10: Do this with a sign-extending shift instead of a
	     zero-extending.  */
	  tem = expand_shift (RSHIFT_EXPR, wider_mode, tem,
			      build_int_2 (size, 0), NULL_RTX, 0);
 ...
 try:
  ...
  if (mode == word_mode)
	...
	{
	  tem = expand_shift (RSHIFT_EXPR, wider_mode, tem,
			      build_int_2 (36, 0), NULL_RTX, unsignedp);
	  return convert_modes (mode, wider_mode, tem, unsignedp);
	}
      else
	...
	if (op1_is_constant)
	  {
		...
		if (d == 1)
		...
		else if (EXACT_POWER_OF_2_OR_ZERO_P (abs_d))
		  {
			...
			&& can_compare_p (GE, compute_mode, ccp_jump))
		      {
		    ...
		    else if (add_optab->handlers[(int) compute_mode].insn_code
			     != CODE_FOR_nothing)
		      {
			rtx t1, t2, t3;
			t1 = expand_shift (RSHIFT_EXPR, compute_mode, op0,
