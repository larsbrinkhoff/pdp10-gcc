  ...
  if (GET_CODE (op0) != CONST_INT || GET_CODE (op1) != CONST_INT
	...
	case AND:
	  ...
	  if (((GET_CODE (op0) == NOT && rtx_equal_p (XEXP (op0, 0), op1))
	      ...
	      && GET_MODE_CLASS (mode) != MODE_CC)
	    return const0_rtx;
	  /* (and (plus (and X C1) C2) C1) -> (and (plus X C2) C1) */
	  if (GET_CODE (op1) == CONST_INT
	      && GET_CODE (op0) == PLUS
	      && GET_CODE (XEXP (op0, 1)) == CONST_INT
	      && GET_CODE (XEXP (op0, 0)) == AND
	      && GET_CODE (XEXP (XEXP (op0, 0), 1)) == CONST_INT
	      && INTVAL (XEXP (XEXP (op0, 0), 1)) == INTVAL (op1))
	    return gen_rtx_AND (mode,
				gen_rtx_PLUS (mode, XEXP (XEXP (op0, 0), 0),
					      XEXP (op0, 1)),
				op1);
#if 0
	  ...
	  if (GET_CODE (op0) == PLUS
	    ...
	    {
	      if (CONSTANT_ADDRESS_P (XEXP (op0, 0))
		  && INTVAL (op1) == 07777777777)
		return XEXP (op0, 0);
	      else
		return gen_rtx_AND (mode, XEXP (op0, 0), op1);
	    }
#endif
	  break;
     ...
     enum machine_mode outermode, innermode;
{
      ...
      if (GET_MODE_SIZE (outermode) < UNITS_PER_WORD)
	{
	  ...
	  rtx extr
	    = gen_rtx_ZERO_EXTRACT (word_mode,
				    ...
				    GEN_INT (BITS_PER_UNIT * byte));
	  return gen_rtx_SUBREG (outermode, extr, 0);
	}
      else
	return adjust_address_nv (op, outermode, byte / UNITS_PER_WORD);
    }
  ...
  if (GET_CODE (op) == PARALLEL)
      ...
      for (i = 0; i < XVECLEN (op, 0); i++)
	{
	  ...
	  if (byte < size)
	    return simplify_gen_subreg (outermode, x, GET_MODE (x),
					byte - size + GET_MODE_SIZE (GET_MODE (x)));
	}
    }
