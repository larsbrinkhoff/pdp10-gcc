     ...
     struct null_pointer_info *npi;
{
  ...
  for (current_block = 0; current_block < n_basic_blocks; current_block++)
	   ...
	   insn = NEXT_INSN (insn))
	{
...
#if 0
	  /* Handle register set to a non-null value.  */
	  if (GET_CODE (reg = SET_DEST (set)) == REG
	      && REGNO (reg) >= npi->min_reg
	      && REGNO (reg) < npi->max_reg
	  ...
	  /* Handle register set from a non-null register.  */
	  if (GET_CODE (reg = SET_DEST (set)) == REG
	      && REGNO (reg) >= npi->min_reg
	      && REGNO (reg) < npi->max_reg
	      && GET_CODE (SET_SRC (set)) == REG
	      && REGNO (SET_SRC (set)) >= npi->min_reg
	      && REGNO (SET_SRC (set)) < npi->max_reg
	      && TEST_BIT (nonnull_local[current_block],
			   REGNO (SET_SRC (set)) - npi->min_reg))
	    SET_BIT (nonnull_local[current_block],
		     REGNO (reg) - npi->min_reg);
#endif
