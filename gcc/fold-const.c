  ...
  *litp = 0;
  ...
  if (TREE_CODE (in) == INTEGER_CST || TREE_CODE (in) == REAL_CST)
    *litp = in;
	    ...
	    /* PDP-10: We can't associatie pointer arithmetic.  */
	    && ! POINTER_TYPE_P (TREE_TYPE (in)))
     ...
     tree lhs, rhs;
{
 ...
 if (!const_p)
  ...
  nbitpos = (lbitpos / nbitsize) * nbitsize; // & ~ (nbitsize - 1);
  lbitpos -= nbitpos;
     ...
     tree truth_type, lhs, rhs;
{
  ...
  lnbitpos = (first_bit / lnbitsize) * lnbitsize; //first_bit & ~ (lnbitsize - 1);
  ...
  if (l_const == 0)
	...
	return 0;
      ...
      rnbitpos = (first_bit / rnbitsize) * rnbitsize; //first_bit & ~ (rnbitsize - 1);
     ...
     tree expr;
{
  ...
  switch (code)
	  ...
	  || TREE_CODE (TREE_OPERAND (t, 0)) == CONVERT_EXPR)
	{
	  ...
	  /* PDP-10: Leave pointer conversions intact.  */
	...
	if (((code0 == RSHIFT_EXPR && code1 == LSHIFT_EXPR)
	    ...
	    && TREE_UNSIGNED (TREE_TYPE (TREE_OPERAND (arg0, 0))))
	  {
	    ...
	    {
	      tree sum = fold (build (PLUS_EXPR, TREE_TYPE (tree01),
				      tree01, tree11));
	      if (TREE_CODE (sum) == INTEGER_CST
		  && TREE_INT_CST_HIGH (sum) == 0
		  && TREE_INT_CST_LOW (sum)
		  == TYPE_PRECISION (TREE_TYPE (TREE_OPERAND (arg0, 0))))
		return build (LROTATE_EXPR, type, TREE_OPERAND (arg0, 0),
			      code0 == LSHIFT_EXPR ? tree01 : tree11);
	    }
