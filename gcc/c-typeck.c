     ...
     int flag;
{
  ...
  switch (code)
...
#if 1
	    if (pdp10_bytesize (TREE_TYPE (result_type)) >= BITS_PER_WORD)
	      inc = fold (build (FLOOR_DIV_EXPR, TREE_TYPE (inc), inc,
				 build_int_2 (UNITS_PER_WORD, 0)));
	    else if (pdp10_bytesize (TREE_TYPE (result_type))
		     == BITS_PER_WORD / 2)
	      inc = fold (build (FLOOR_DIV_EXPR, TREE_TYPE (inc), inc,
				 build_int_2 (2, 0)));
#else
	    if (tree_low_cst (inc, 1) >= UNITS_PER_WORD)
	      inc = fold (build (FLOOR_DIV_EXPR, TREE_TYPE (inc), inc,
				 build_int_2 (UNITS_PER_WORD, 0)));
#endif
	  }
	...
	warning ("address of scalar %d-bit byte taken",
		 tree_low_cst (TYPE_SIZE (TREE_TYPE (arg)), 1));
	...
	if (TREE_CODE (arg) == COMPONENT_REF)
	  {
	    ...
	    {
	      tree byte_pos = byte_position (field);
	      tree size_unit, size_bits;

	      /* PDP-10: support the popular offsetof() macro
		 definition: &((struct X *)0)->M.  */
	      if (TREE_CODE (addr) == INTEGER_CST
		  && compare_tree_int (addr, 0) == 0)
		return convert (argtype, byte_pos);

	      if (TREE_CODE (TREE_TYPE (field)) == ARRAY_TYPE)
		{
		  size_unit = TYPE_SIZE_UNIT (TREE_TYPE (TREE_TYPE (field)));
		  size_bits = TYPE_SIZE (TREE_TYPE (TREE_TYPE (field)));
		}
	      else
		{
		  size_unit = DECL_SIZE_UNIT (field);
		  size_bits = DECL_SIZE (field);
		}

	      if (size_unit == NULL_TREE
		  && TREE_CODE (TREE_TYPE (field)) == ARRAY_TYPE)
		{
	      ...
	      if (tree_low_cst (size_bits, 1) == 7)
		{
		  byte_pos = fold (build (FLOOR_DIV_EXPR, TREE_TYPE (byte_pos),
					  byte_pos, build_int_2 (4, 0)));
		  byte_pos = fold (build (MULT_EXPR, TREE_TYPE (byte_pos),
					  byte_pos, build_int_2 (5, 0)));
		}

	      if (TREE_CODE (addr) == PLUS_EXPR
		  && TREE_CODE (TREE_OPERAND (addr, 1)) == INTEGER_CST)
		{
		  tree offset = TREE_OPERAND (addr, 1);
		  tree size_unit;
		  ...
		  if (TREE_CODE (TREE_TYPE (addr)) != POINTER_TYPE)
		    abort ();
		  size_unit = TYPE_SIZE_UNIT (TREE_TYPE (TREE_TYPE (addr)));

		  if (tree_low_cst (size_unit, 1) >= 4)
		    offset = fold (build (MULT_EXPR, TREE_TYPE (offset),
					  offset, build_int_2 (4, 0)));
		  else if (tree_low_cst (size_unit, 1) == 2)
		    offset = fold (build (MULT_EXPR, TREE_TYPE (offset),
					  offset, build_int_2 (2, 0)));
		  byte_pos = fold (build (PLUS_EXPR, TREE_TYPE (byte_pos),
					  byte_pos, offset));
		}

	      if (tree_low_cst (size_unit, 1) >= UNITS_PER_WORD)
		byte_pos = fold (build (FLOOR_DIV_EXPR, TREE_TYPE (byte_pos),
					byte_pos,
					build_int_2 (UNITS_PER_WORD, 0)));
	      else if (tree_low_cst (size_unit, 1) == 2)
		byte_pos = fold (build (FLOOR_DIV_EXPR, TREE_TYPE (byte_pos),
					byte_pos, build_int_2 (2, 0)));

	      addr = fold (build (PLUS_EXPR, argtype,
				  convert (argtype, addr),
				  convert (argtype, byte_pos)));
	    }
	  }
	else
...
#if 1
	/* PDP-10: differently sized char types are not considered
	   incompatible in pointer assignments.  */
	if (tree_low_cst (TYPE_SIZE (ttl), 1) > 9
	    || tree_low_cst (TYPE_SIZE (ttr), 1) > 9)
#endif
  ...
  if (code == ARRAY_TYPE)
	  ...
	  && ((inside_init && TREE_CODE (inside_init) == STRING_CST)))
	{
	  ...
	  if ((TYPE_MAIN_VARIANT (TREE_TYPE (TREE_TYPE (inside_init)))
	      ...
	      /* PDP-10: Account for other PDP-10 char types.  */
	      && TYPE_PRECISION (TREE_TYPE (TREE_TYPE (inside_init))) > 9)
	    {
