...
/* Type of computer arithmetic.
   Only one of DEC, IBM, IEEE, C4X, PDP10, or UNK should get defined.
   ...
   `PDP10' refers specifically to the Digital Equipment Corp PDP-10
   floating point data structure.  This model currently supports no
   type wider than DFmode.  The PDP10 conversions were contributed by
   lars@nocrew.org (Lars Brinkhoff) and funded by XKL, LLC.
...
#if TARGET_FLOAT_FORMAT == PDP10_FLOAT_FORMAT
#define PDP10
#else /* not even PDP10 */
...
#if !defined(DEC) && !defined(IBM) && !defined(C4X) && !defined(PDP10)
#define INFINITY
#define NANS
#endif
...
do {									\
     if (HOST_FLOAT_WORDS_BIG_ENDIAN == REAL_WORDS_BIG_ENDIAN)		\
       e53toe ((const unsigned EMUSHORT *) (r), (e));			\
     else								\
       {								\
	 unsigned EMUSHORT w[5];					\
         memcpy (&w[4], ((EMUSHORT *) r), sizeof (EMUSHORT));		\
         memcpy (&w[3], ((EMUSHORT *) r) + 1, sizeof (EMUSHORT));	\
         memcpy (&w[2], ((EMUSHORT *) r) + 2, sizeof (EMUSHORT));	\
         memcpy (&w[1], ((EMUSHORT *) r) + 3, sizeof (EMUSHORT));	\
         memcpy (&w[0], ((EMUSHORT *) r) + 4, sizeof (EMUSHORT));	\
	 e53toe (w, (e));						\
       }								\
   } while (0)
...
do {									\
	 ...
	 unsigned EMUSHORT w[5];					\
	 etoe53 ((e), w);						\
         memcpy (((EMUSHORT *) r), &w[4], sizeof (EMUSHORT));		\
         memcpy (((EMUSHORT *) r) + 1, &w[3], sizeof (EMUSHORT));	\
         memcpy (((EMUSHORT *) r) + 2, &w[2], sizeof (EMUSHORT));	\
         memcpy (((EMUSHORT *) r) + 3, &w[1], sizeof (EMUSHORT));	\
         memcpy (((EMUSHORT *) r) + 4, &w[0], sizeof (EMUSHORT));	\
       }								\
   } while (0)

#else /* LONG_TYPE_SIZE != 72 */
...
#endif
#ifdef PDP10
static void pdp10negate	PARAMS ((unsigned EMUSHORT *x,
				 enum machine_mode));
static void pdp10toe	PARAMS((const unsigned EMUSHORT *, unsigned EMUSHORT *,
				enum machine_mode));
static void etopdp10	PARAMS((const unsigned EMUSHORT *, unsigned EMUSHORT *,
				enum machine_mode));
static void topdp10	PARAMS((unsigned EMUSHORT *, unsigned EMUSHORT *,
				enum machine_mode));
#endif
#if 0
...
#else
#ifdef PDP10
/* Convert e-type X to PDP-10 format double E.  */

static void
etoe53 (x, e)
     const unsigned EMUSHORT *x;
     unsigned EMUSHORT *e;
{
  etopdp10 (x, e, DFmode);
}
...
/* Convert exploded e-type X, that has already been rounded to
   62-bit precision, to PDP-10 double Y.  */
...
#else  /* it's neither DEC nor IBM nor C4X nor PDP10 */
...
/* Convert e-type X to PDP-10 float E.  */
...
/* Convert exploded e-type X, that has already been rounded to
   float precision, to PDP-10 float Y.  */
  ...
  if (mode == SFmode)
      ...
      for (i = 4; i >= 0; i--)
	{
	  if (i == 2)
	    {
	      y = (x[2] & 0x07ff) + carry;
	      carry = y > 0x07ff ? 0x1000 : 0;
	      x[2] = (x[2] & 0xf000) + (y & 0x07ff);
	    }
...
/* Convert PDP-10 single/double precision to e type.  */

static void
pdp10toe (d, e, mode)
     const unsigned EMUSHORT *d;
     unsigned EMUSHORT *e;
     enum machine_mode mode;
{
  ...
  if (d[0] == 0x0000
      && d[1] == 0x0000
      && ((mode == SFmode && (d[2] & 0xf000) == 0x0000)
	  || (d[2] == 0x0000
	      && d[3] == 0x0000
	      && (d[4] & 0xff00) == 0x0000)))
    {
      memset (e, 0, NE * sizeof (EMUSHORT));
      return;
    }

  ecleaz (y);			/* start with a zero */
  if (d[0] & (unsigned int) 0x8000)
    {
      pdp10negate (d, mode);
      y[0] = 0xffff;		/* fill in our sign */
    }
...
/* Convert e type to PDP-10 single/double precision.  */
     ...
     enum machine_mode mode;
{
  ...
  /* Round off to nearest or even. */
  rndsav = rndprc;
  rndprc = mode == SFmode ? 27 : 62;
  emdnorm (xi, 0, 0, exp, mode == SFmode ? 27 : 62);
  rndprc = rndsav;
  topdp10 (xi, d, mode);
}
...
#if TARGET_FLOAT_FORMAT == PDP10_FLOAT_FORMAT
    /* Imaginary XFmode, TFmode, and OFmode formats.  */
  case 108:
  case 144:
  case 288:
    return 59;
