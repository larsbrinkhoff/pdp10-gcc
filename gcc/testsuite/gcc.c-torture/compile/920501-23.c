...
poi_si(p)si*p;{si x;for(;;){x=*p++;if(x)return x;}}
pri_si(p)si*p;{si x;for(;;){x=*++p;if(x)return x;}}
poi_di(p)di*p;{di x;for(;;){x=*p++;if(x)return x;}}
pri_di(p)di*p;{di x;for(;;){x=*++p;if(x)return x;}}
