...
static short r_order[(2 * 10 * (2 + 1))];
int r_spill_index[(2 * 10 * (2 + 1))];
extern struct reload rld[(2 * 10 * (2 + 1))];
static rtx *reg_last_reload_reg;
static HARD_REG_SET rr_valid;
static HARD_REG_SET rr_dead;
static HARD_REG_SET rr_died;
     ...
     struct insn_chain *chain;
{
  ...
  for (j = 0; j < n_reloads; j++)
    {
      register int r = r_order[j];
      register int i = r_spill_index[r];
