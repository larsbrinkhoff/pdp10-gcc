     ...
     const struct loop *loop;
{
  ...
  /* PDP-10: If the loop is counting down while > or >= 0 (or
     equivalent), doloop isn't helping.  TODO: Maybe the right
     solution is to remove the doloop_end pattern entirely from
     pdp10.md.  */
  if (loop_info->comparison_code == GT
      && GET_CODE (loop_info->final_value) == CONST_INT
      && (INTVAL (loop_info->final_value) == 0
	  || INTVAL (loop_info->final_value) == -1)
      && GET_CODE (loop_info->increment) == CONST_INT
      && INTVAL (loop_info->increment) == -1)
    return 0;
